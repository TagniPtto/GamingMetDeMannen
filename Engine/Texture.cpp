...
void* Texture::GetPixel() const
{
	return pLoadedSurface->pixels;
}
...
{
	m_Width = float(pLoadedSurface->w);
	m_Height = float(pLoadedSurface->h);
	...
	glBindTexture(GL_TEXTURE_2D, m_Id);
	glPixelStorei(GL_UNPACK_ROW_LENGTH, pLoadedSurface->pitch / pLoadedSurface->format->BytesPerPixel);
	// check for errors. Can happen if a texture is created while a static pointer is being initialized, even before the call to the main function.
	GLenum e = glGetError();
	if (e != GL_NO_ERROR)
	{
